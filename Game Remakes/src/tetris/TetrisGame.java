...
public class TetrisGame extends JPanel {
    ...
    public TetrisGame(ScreenManager manager) {
        ...
        addComponentListener(new ComponentAdapter() {
        	public void componentResized(ComponentEvent event) {
        		sideBar.setPreferredSize(new Dimension(getWidth() / 3, getHeight()));
    ...
    public boolean checkPlaced(Piece piece) {
        ...
        Position[] placedPositions = placedPieces.stream()
                ...
                .flatMap(pos -> Arrays.stream(pos).filter(x -> x.getCol() >= furthestLeft && x.getCol() <= furthestRight))
    ...
    private boolean checkOverlap(Position[] possiblePositions, Position[] bannedPositions) {
    	...
    	for (int i = 0; i < possiblePositions.length; i++) {
    		...
    		int matches = (int) Arrays.stream(bannedPositions).filter(x -> x.equals(current)).count();
    ...
    protected void paintComponent(Graphics g) {
        ...
        squareSize = Math.min(getWidth(), getHeight()) / Math.max(rows, cols);
    ...
    public void keyPressed(KeyEvent e) {
        if (inGame) {
            ...
            boolean moved = false;
            if (keyCode == KeyEvent.VK_A || keyCode == KeyEvent.VK_LEFT) { //move piece left
